...
class Events_model extends CI_Model
{
	...
	public function getAllRows($data,$start,$limit){
		$d = date('Y-m-d');
	...
	public function getAllEvents(){
		$d = date('Y-m-d');
		$this->db->where("start_date <=",$d);
		$this->db->where("end_date >=",$d);
		$this->db->select('*');
		$query = $this->db->get('events');
		if($query->num_rows())
		{
	...
	public function getAllEventsByCategories($categories,$producers){
		$categories[]	= 0;
		$producers[]	= 0;
		...
		$this->db->where('type','producers');
		$this->db->where_in("category_id",$producers);
		$this->db->select('*');
		$query = $this->db->get('events');
		//echo $this->db->last_query();
	...
	function get_options($parent_id=0,&$indent=''){
		...
		$query  = "select * from events where parent_id=$parent_id";
