...
class Documents_model extends CI_Model
{
	...
	public function insertTextMessage($data){
		$this->db->insert('message_documents',$data);
		//echo $this->db->last_query();
		return true;
	}
	...
	*/
	...
	public function getTextByField($field,$id){
		...
		$query  =   $this->db->get('message_documents');
		//echo $this->db->last_query();
		if($query->num_rows())
		{
	...
	public function assignJob($id,$priority_id){
		...
		if($query->num_rows()){
			...
			$this->db->where("is_responded",0);
			$this->db->where("is_sender_deleted",0); //not deleted
			$query2 = $this->db->get('message_documents');
			if($query2->num_rows()<1){
			$this->db->where('id',$row[0]->id);
			$arr['translator_id'] = $id;
			$status = $this->db->update('message_documents',$arr);
				if($status){
					return $row[0];
				}
	...
	public function update($data,$id){
		...
		$this->db->where('id',$id);
		$this->db->update('message_documents',$data);
		return true;
	}
	...
	public function getTranslatorId($data){
		$this->db->select('message_documents.translator_id');
	...
	public function getTraslatorRating($id, $IDs){
		$this->db->select('message_documents.id');
		$this->db->where('translator_id',$id);
		$this->db->where('type','document');
		$this->db->where('is_responded',1);
		$query = $this->db->get('message_documents');
	...
	public function countAllTranslatorRespondedMessages($data,$sender_id,$table_name,$priorty){
	...
	public function getTranslatorPendingMessages($table_name,$priorty){
	...
	public function downloadData($array,$user_id,$table_name){
		$this->db->select('file,response_file');
	...
	public function checkForDownload($fileName,$user_id,$table_name,$tranlator){
		if($tranlator){
			$where = "( is_responded=0 ) or (is_responded!=0 and translator_id=".$user_id.")";
		}
		else{
			$where = "(response_file='".$fileName."') and (sender_id=".$user_id." or receiver_id=".$user_id.")";
		}
		//echo $where;die();
	...
	public function checkForTranslator($user_id,$table_name){
