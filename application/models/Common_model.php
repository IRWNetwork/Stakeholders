...
class Common_model extends CI_Model
{
	...
	function save_braintree_merchant_info_update($data, $id) {

		$this->db->where('id', $id);
		$this->db->update('braintree_merchant_info', $data); 

		//echo $this->db->last_query();exit;
		return true;
	}

	function save_braintree_merchant_info_save($data) {
		$this->db->set($data); 
		$this->db->insert('braintree_merchant_info'); 
		//echo $this->db->last_query();exit;
		return true;
	}
	...
	public function saveCSV(){
			...
			if($_FILES["file"]["size"] > 0)
			{
				...
				while (($getData = fgetcsv($file, 10000, ",")) !== FALSE)
				{
					...
					//$data =  $getData[0]." ".$getData[1]." ".$getData[2]." ".$getData[3]." ".$getData[4]." ".$getData[5].$getData[6].$getData[7]."</br>";
                    if($count>0){
						$data = array(
							'state'=>$getData[0],
							'city'=>$getData[1],
							...
							'status'=>$getData[3],
							'owner'=>$getData[4],
							'established'=>$getData[5],
							'website'=>$getData[6],
							'notes'=>$getData[7]
						);
						$query = $this->db->insert('site_feature_checklist', $data);
					}
					$count += 1;
				}
				fclose($file);
